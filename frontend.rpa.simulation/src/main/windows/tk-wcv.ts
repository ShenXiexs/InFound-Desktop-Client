...
export class TkWcv {
  ...
  private isAffiliateHomepageReady = false
  private lastShopRegion = 'MX'
  ...
  public startSellerLoginSuccessMonitor(): void {
    ...
    this.isAffiliateHomepageReady = false
  ...
  private async checkSellerLoginSuccess(): Promise<void> {
    ...
    try {
      ...
      this.isAffiliateHomepageReady = true
    ...
    try {
      const result = await wc.executeJavaScript(
        `(() => {
          ...
          return {
            href: location.href,
            hasSuccessText: bodyText.includes(${JSON.stringify(this.loginSuccessText)})
          }
        })()`
      )
      const href = String(result?.href || '')
      ...
      const ready = Boolean(result?.hasSuccessText) || href.startsWith('https://affiliate.tiktok.com/')
      if (ready && region) {
        this.isAffiliateHomepageReady = true
      }
      return this.isAffiliateHomepageReady
    } catch {
      return false
    }
  }
