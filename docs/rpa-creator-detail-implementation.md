...
1. 应用已启动。
2. 已执行 `login`，并且已检测到登录成功。

如果现在要抓取达人 `7494007128896931516` 的详情页，终端直接输入：
...
```text
https://affiliate.tiktok.com/connection/creator/detail?cid=7494007128896931516&shop_region=<region>
```
...
| Est. post rate | 卡片标题 `Est. post rate`，值 `61.95%` | `getMetricCardValue('Est. post rate')` | `est_post_rate` |
...
| LIVE GPM | 卡片标题 `LIVE GPM`，值 `MX$0.00` | `getMetricCardValue('LIVE GPM')` | `live_gpm` |
| LIVE streams | 卡片标题 `LIVE streams`，值 `0` | `getMetricCardValue('LIVE streams')` | `live_streams` |
| Avg. LIVE views | 卡片标题 `Avg. LIVE views`，值 `2` | `getMetricCardValue('Avg. LIVE views')` | `avg_live_views` |
...
| Avg. video likes | 第 2 个右箭头展开后，卡片标题 `Avg. video likes`，值 `234` | `clickArrowByIndex(1, 'Avg. video likes')` + `getMetricCardValue('Avg. video likes')` | `avg_video_likes` |
| Avg. video comments | 第 2 个右箭头展开后，卡片标题 `Avg. video comments`，值 `1` | `clickArrowByIndex(1, 'Avg. video likes')` + `getMetricCardValue('Avg. video comments')` | `avg_video_comments` |
| Avg. video shares | 第 2 个右箭头展开后，卡片标题 `Avg. video shares`，值 `7` | `clickArrowByIndex(1, 'Avg. video likes')` + `getMetricCardValue('Avg. video shares')` | `avg_video_shares` |
| Avg. LIVE likes | 第 3 个右箭头展开后，卡片标题 `Avg. LIVE likes`，值 `0` | `clickArrowByIndex(2, 'Avg. LIVE likes')` + `getMetricCardValue('Avg. LIVE likes')` | `avg_live_likes` |
| Avg. LIVE comments | 第 3 个右箭头展开后，卡片标题 `Avg. LIVE comments`，值 `0` | `clickArrowByIndex(2, 'Avg. LIVE likes')` + `getMetricCardValue('Avg. LIVE comments')` | `avg_live_comments` |
| Avg. LIVE shares | 第 3 个右箭头展开后，卡片标题 `Avg. LIVE shares`，值 `0` | `clickArrowByIndex(2, 'Avg. LIVE likes')` + `getMetricCardValue('Avg. LIVE shares')` | `avg_live_shares` |
...
3. 读基础资料
4. 先读 `brands_list`
5. 依次点击第 1、2、3 个右箭头展开补充字段
6. 读取所有指标卡
7. 读取 4 个 legend JSON
8. 读取 `videos_list`
9. 读取 `videos_with_product`
10. 读取 `relative_creators`
...
```json
{
  ...
  "brands_list": "TIMEOFF,Maybelline New York,...",
